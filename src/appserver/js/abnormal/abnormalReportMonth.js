...
Ext.onReady(function() {
                ...
                listeners : {
                    "click" : function() {
                        ...
                        if (reportYear == '点击输入年' || reportYear == '') {
                            Ext.Msg.alert("提示", "请输入年份！如:2012");
                            return;
                        }
                        var reportMonth = Ext.fly("reportMonth").dom.value;
                        if (reportMonth == '点击输入月份' || reportMonth == '') {
                            Ext.Msg.alert("提示", "请输入月份！如:03或者3");
                            return;
                        }
                        var time= reportYear+'年'+reportMonth+ "月的违规次数";
                        ...
                        chartStore1.setBaseParam("month", reportMonth);
                        chartStore1.load();
                        chartStore2.setBaseParam("year", reportYear);
                        chartStore2.setBaseParam("month", reportMonth);
                        chartStore2.load();
                    }
                }
            }]
    ...
    var proxy = new Ext.data.HttpProxy({
        url:"../sysAbnormalService.do?m=selectMonthForAbnormal",
        timeout: 20*60*1000
    });
...
//        boxM,
    ...
    var chartStore1 = new Ext.data.JsonStore({
        url : '../sysAbnormalService.do?m=selectAbnormalMonth',
        root : 'rows',
...
//        url : '../sysAbnormalService.do?m=selectLineChartYear',
        url : '../sysAbnormalService.do?m=selectLineChartMonth',
        root : 'rows',
