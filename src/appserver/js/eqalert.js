...
Ext.onReady(function() {
           ...
           /* listeners:{
			...
			listeners : {
				"click" : function() {
                    ...
                    var value1 = Ext.getCmp("endDate").getValue();
                    var value2 = Ext.getCmp("startDate").getValue();//①
                    var dt1 = new Date(value1);//获取选择的时期对象
                    var dt2 = new Date(value2);//获取当前日期的时间对象
                    value1 = Date.parse(dt1); //Date.parse的处理很关键
                    value2 = Date.parse(dt2);
                    if (value1 < value2) {
                        Ext.Msg.alert('信息', '开始日期必须小于结束日期');
                        Ext.getCmp("startDate").setValue("");
	...
	var store = new Ext.data.JsonStore({
		...
		fields : ['id', 'alertTime', 'equName', 'alertInfo', 'ip', 'isRead']
