Ext.onReady(function(){
    ...
    var proxy_ips = new Ext.data.HttpProxy({
        url:"configManager.do?m=readIps"
    });
    ...
    store_ips.addListener('load',function(){
        ...
        if(ip1==null||ip1.length==0){
            Ext.getCmp('ip1.info').setValue("0.0.0.0");
        }else {
            Ext.getCmp('ip1.info').setValue(ip1);
        }
        if(ip2==null||ip2.length==0){
            Ext.getCmp('ip2.info').setValue("0.0.0.0");
        } else{
        ...
        handler: function() {
            if(form_1.form.isValid()){
                form_1.getForm().submit({
                    url:'configManager.do?m=update8443',
                    method:'POST',
        ...
        handler: function() {
            if(form_2.form.isValid()){
                form_2.getForm().submit({
                    url:'configManager.do?m=update8000',
                    method:'POST',
                    success: function(form,action) {
                        var flag = action.result.msg;
                        Ext.MessageBox.show({
