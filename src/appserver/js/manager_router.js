...
/********************************************* -- internal_grid_panel start -- *******************************************************************************************************/
    ...
    var proxy = new Ext.data.HttpProxy({
       url:"interfaceManager.do?m=readRouter"
    });
...
function insert_win(grid,store){
    var name_store = new Ext.data.Store({
        url:"interfaceManager.do?m=readInterfaceName&t=router",
        reader:name_reader
    });
    name_store.load();
                ...
                handler : function() {
                    if (formPanel.form.isValid()) {
                        formPanel.getForm().submit({
                            url :'interfaceManager.do?m=saveRouter',
                            method :'POST',
                            waitTitle :'系统提示',
			                waitMsg :'正在保存...',
    ...
    if(count==0){
        ...
        Ext.MessageBox.show({
            ...
            fn:function(e){
                if(e == 'ok'){
                    Ext.Ajax.request({
                        url : 'interfaceManager.do?m=deleteRouter',             // 删除 连接 到后台
