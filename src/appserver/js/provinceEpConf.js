...
Ext.onReady(function() {
				...
				if (selectedRows.length == 0) {
					...
					Ext.Msg.confirm('警告', '确定要删除这些记录吗？', function(btn) {
						if (btn == 'yes') {
							...
							Ext.Ajax.request({
								url : '../provinceEpConf.do?m=delete',
								params : {
									ids : ids
								},
	...
	function showAddWindow() {
		...
		editWindow.show();
		editWindow.load('../provinceEpConf.do?m=showAdd');
	}
...
function showDetail() {
	...
	editWindow.removeAll(true);
	editWindow.show();
	editWindow.load('../provinceEpConf.do?m=detail&id=' + selectGridRow.data.id);
}
...
function showUpdate() {
	...
	editWindow.show();
	editWindow.load({
		url : '../provinceEpConf.do?m=showUpdate&id=' + selectGridRow.data.id,
		callback : function() {
			init();
		},
		scripts : true
