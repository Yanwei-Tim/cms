 ...
 *                scope="request" path="/intLinkConf" validate="false"
 *                parameter="m"
 * @struts.action-forward name="indexJSON"
 *                        path="/WEB-INF/pages/conf/intLinkIndexJSON.jsp"
 * @struts.action-forward name="add" path="/WEB-INF/pages/conf/intLinkAdd.jsp"
 * @struts.action-forward name="update"
 *                        path="/WEB-INF/pages/conf/intLinkUpdate.jsp"
 * @struts.action-forward name="detail"
 *                        path="/WEB-INF/pages/conf/intLinkDetail.jsp"
 */
public class IntLinkConfAction extends DispatchActionSupport {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(IntLinkConfAction.class);
	...
	public ActionForward index(ActionMapping mapping, ActionForm form,
		...
		Map model = confService.getIntLinkConfIndexModel(pageIndex);
	...
	public ActionForward detail(ActionMapping mapping, ActionForm form,
		...
		Map model = confService.getIntLinkDetailModel(id);
		request.setAttribute("model", model);
	...
	public ActionForward showAdd(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		Map model = confService.getIntLinkAddModel();
		request.setAttribute("model", model);
	...
	public ActionForward add(ActionMapping mapping, ActionForm form,
		...
		if(account !=null){
			...
			try {
				IntLink entry = new IntLink();
				RequestUtils.populate(entry, request);
				linkName = entry.getLinkName();
				confService.newIntLink(entry);
				logService.newLog("INFO", account.getUserName(), "链路配置", "内部链路 " + linkName + " 新增成功");
				msg = "新增成功";
			} catch (Exception e) {
				logger.error("", e);
				logService.newLog("ERROR", account.getUserName(), "链路配置", "内部链路 " + linkName + " 新增不成功");
				msg = "新增失败";
			}
		}
	...
	public ActionForward showUpdate(ActionMapping mapping, ActionForm form,
			...
			throws Exception {
		Long id = ServletRequestUtils.getLongParameter(request, "id");
		Map model = confService.getIntLinkUpdateModel(id);
		request.setAttribute("model", model);
	...
	public ActionForward update(ActionMapping mapping, ActionForm form,
		...
		if(account !=null){
			...
			try {
				...
				RequestUtils.populate(entry, request);
				linkName = entry.getLinkName();
				confService.updateIntLink(entry);
				logService.newLog("INFO", account.getUserName(), "链路配置", "内部链路 " + linkName + " 修改成功");
				msg = "修改成功";
			} catch (Exception e) {
				logger.error("", e);
				logService.newLog("ERROR", account.getUserName(), "链路配置", "内部链路 " + linkName + " 修改不成功");
				msg = "修改失败";
			}
		}
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		if(account !=null){
			...
			try{
				confService.deleteIntLinks(idsArr, account);
				msg = "删除成功";
			}catch (Exception e) {
				msg = "删除失败";
			}
    ...
    public ActionForward showLink(ActionMapping mapping, ActionForm form,
                                HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        String json =confService.showLinkInt();
        response.setCharacterEncoding("UTF-8");
        response.setContentType("text/html");
        PrintWriter writer = response.getWriter();
        writer.println(json);
